...
// Remove the wrongly injected block
const startBad = content.indexOf('  "Manage primary emergency contact & synchronous dual-SMS dispatch');
const endBad = content.indexOf('export function translateInline');

if (startBad !== -1 && endBad !== -1) {
  content = content.substring(0, startBad) + content.substring(endBad);
}

// Inject correctly before `};\nexport function`
const injectPointCorrect = "};\n\nexport function translateInline";
// or just find `};` before `export`
