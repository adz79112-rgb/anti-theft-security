...
export default function App() {
  ...
  // Never allow minimizing, exiting, or closing the app during active theft mode or emergency lockdown
  useEffect(() => {
    let backHandle: any = null;
    if (Capacitor.isNativePlatform()) {
      CapApp.addListener('backButton', ({ canGoBack }) => {
        if (isTheftModeTriggered || isStealthStolenModeOpen || showPowerOffModal) {
          console.log('[Security] Back button blocked during active theft/lockdown mode.');
          return;
        }
        if (canGoBack) {
          window.history.back();
  ...
  }, [isTheftModeTriggered, isStealthStolenModeOpen, showPowerOffModal]);
  ...
  const executeTheftTrigger = useCallback(
    async (senderNumber: string, directBlackScreen: boolean = false) => {
      ...
      // 1. Trigger the visual theft overlay IMMEDIATELY so the user is never left waiting
      ...
      // 2. Perform camera captures, GPS locating, and alert dispatches asynchronously in background
      (async () => {
        // Silently trigger GPS hardware in background so it warms up
